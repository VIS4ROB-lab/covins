...
const brisk::Hamming& distance_ = brisk::Hamming();
...
{
    //double dist = cv::norm(a, b, cv::NORM_HAMMING);
    //return dist;

    double dist = distance_(a.data,b.data,48);
    return dist;
...
double FBRISK::distanceA(const cv::Mat &a, const cv::Mat &b)
{
    double dist = distance_(a.data,b.data,48);
    return dist;
}
