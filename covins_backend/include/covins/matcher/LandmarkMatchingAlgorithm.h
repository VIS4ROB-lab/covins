...
*/
/*
 * LandmarkMatchingAlgorithm.hpp
 * @brief Header for the LandmarkMatchingAlgorithm class (3d-3d matching).
 * @author: Marco Karrer
 ...
 */
class LandmarkMatchingAlgorithm : public MatchingAlgorithm {
 public:
  ...
  LandmarkMatchingAlgorithm(float distanceThreshold);

  virtual ~LandmarkMatchingAlgorithm();
  ...
  virtual float distance(size_t indexA, size_t indexB) const {
    const float dist = static_cast<float>(
        FeatureMatcher::DescriptorDistanceHamming(
            kfPtrA_->GetDescriptorCV(indexA),
